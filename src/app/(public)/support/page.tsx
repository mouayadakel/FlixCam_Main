...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
  title: t(locale, 'seo.supportTitle'),
  description: t(locale, 'seo.supportDescription'),
        ...
        <PublicContainer className="max-w-3xl">
          ...
          <div className="mb-12 grid gap-6 md:grid-cols-2">
            <div className="rounded-2xl border border-border-light/60 bg-surface-light/30 p-6 shadow-card">
              <h2 className="mb-2 flex items-center gap-2 font-semibold text-text-heading">
                ...
                {t(locale, 'supportPage.phoneAndWhatsapp')}
              </h2>
              <p className="mb-4 text-sm text-text-muted">{t(locale, 'supportPage.phoneDesc')}</p>
              <Button asChild size="lg" className="w-full">
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">
                  <MessageCircle className="ms-2 h-5 w-5" />
                  {t(locale, 'supportPage.contactWhatsapp')}
                </a>
              </Button>
            ...
            <div className="rounded-2xl border border-border-light/60 bg-surface-light/30 p-6 shadow-card">
              <h2 className="mb-2 flex items-center gap-2 font-semibold text-text-heading">
                ...
                {t(locale, 'supportPage.emailTitle')}
              </h2>
              <p className="mb-4 text-sm text-text-muted">{t(locale, 'supportPage.emailDesc')}</p>
              <a
          ...
          <div className="mb-12 rounded-2xl border border-border-light/60 bg-surface-light/30 p-6 shadow-card">
            <h2 className="mb-2 flex items-center gap-2 font-semibold text-text-heading">
              ...
              {t(locale, 'supportPage.faqTitle')}
            </h2>
            <p className="mb-4 text-sm text-text-muted">{t(locale, 'supportPage.faqDesc')}</p>
            <Button asChild variant="outline" size="lg">
              <Link href="/faq">{t(locale, 'supportPage.browseFaq')}</Link>
            </Button>
          </div>
          ...
          <div className="rounded-2xl border border-brand-primary/20 bg-brand-primary/5 p-6 text-center">
            <h2 className="mb-2 text-xl font-bold text-text-heading">
              {t(locale, 'supportPage.notFoundTitle')}
            </h2>
            <p className="mb-4 text-sm text-text-muted">{t(locale, 'supportPage.notFoundDesc')}</p>
            <Button asChild size="lg">
              <Link href="/contact">{t(locale, 'supportPage.contactUs')}</Link>
            </Button>
          </div>

          <p className="mt-8 text-center text-sm text-text-muted">
            {t(locale, 'supportPage.seeAlso')}{' '}
            <Link
              href="/policies"
              className="font-medium text-brand-primary underline hover:opacity-90"
            >
              {t(locale, 'supportPage.termsLink')}
            </Link>{' '}
            <Link
              href="/how-it-works"
              className="font-medium text-brand-primary underline hover:opacity-90"
            >
              {t(locale, 'supportPage.howItWorksLink')}
            </Link>
            .
          </p>
        </PublicContainer>
