...
export async function PATCH(
  ...
  try {
    ...
    if (nameAr !== undefined) data.nameAr = typeof nameAr === 'string' ? nameAr.trim() || null : null
    if (baseFee !== undefined) data.baseFee = typeof baseFee === 'number' ? baseFee : parseFloat(baseFee)
    if (perKmFee !== undefined) data.perKmFee = typeof perKmFee === 'number' ? perKmFee : perKmFee == null ? null : parseFloat(perKmFee)
    if (minOrder !== undefined) data.minOrder = typeof minOrder === 'number' ? minOrder : minOrder == null ? null : parseFloat(minOrder)
    if (leadTimeHrs !== undefined) data.leadTimeHrs = typeof leadTimeHrs === 'number' ? leadTimeHrs : parseInt(leadTimeHrs, 10)
