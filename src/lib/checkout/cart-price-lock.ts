/**
 * Ensure a cart has a valid price lock before checkout payment.
 */
...
async function getLockTtlMinutes(): Promise<number> {
  ...
  return Math.max(1, Math.min(480, configuredTtl || DEFAULT_LOCK_TTL_MINUTES))
}

function buildPricingItems(cart: CartWithItems): PricingItem[] {
...
export async function ensureCartPriceLock(
  cart: CartWithItems,
  customerId: string | null
): Promise<{ expiresAt: Date; totalAmount: number }> {
  const existing = await prisma.priceLock.findUnique({
  ...
  const pricingItems = buildPricingItems(cart)
  ...
  const lockTtlMinutes = await getLockTtlMinutes()
