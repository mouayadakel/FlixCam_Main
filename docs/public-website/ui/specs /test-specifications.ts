    ...
    if (isStructuredSpecifications(item.specifications)) {
      ...
      try {
        ...
        const converted = convertFlatToStructured(
          item.specifications as FlatSpecifications,
          {
...
function generateRecommendations(results: TestResults) {
  ...
  if (summary.flat > 0) {
    ...
    const successRate = conversionTests.attempted > 0
    ...
    if (conversionTests.failed.length > 0) {
      results.recommendations.push(
        `❌ ${conversionTests.failed.length} flat conversions failed - ` +
...
function printResults(results: TestResults, totalTime: number) {
  ...
  if (results.conversionTests.attempted > 0) {
    ...
    if (results.conversionTests.attempted > 0) {
      const successRate = (
        (results.conversionTests.successful / results.conversionTests.attempted) * 100
      ).toFixed(1);
  ...
  if (results.validationTests.tested > 0) {
    ...
    if (results.validationTests.tested > 0) {
      const passRate = (
        (results.validationTests.passed / results.validationTests.tested) * 100
      ).toFixed(1);
  ...
  console.log(`  Avg Conversion Time: ${formatDuration(results.performanceMetrics.avgConversionTime)}`);
  console.log(`  Avg Validation Time: ${formatDuration(results.performanceMetrics.avgValidationTime)}`);
