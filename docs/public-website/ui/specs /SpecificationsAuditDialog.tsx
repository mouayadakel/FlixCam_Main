    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {items.map(item => (
          <div
            key={item.id}
            className="flex items-start gap-3 p-4 rounded-lg border border-border-light bg-white hover:border-brand-primary/30 transition-colors"
          >
            {/* Checkbox */}
            {showActions && (
              <Checkbox
            ...
            <a
              ...
              className="shrink-0 p-2 rounded hover:bg-surface-light transition-colors"
            >
    ...
    <>
      <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        <DialogContent className="max-w-5xl max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              فحص حالة المواصفات لجميع المعدات في النظام
            </DialogDescription>
          </DialogHeader>
                ...
                <Tabs defaultValue="needsConversion">
                  <TabsList className="w-full grid grid-cols-4">
                    ...
                    <TabsTrigger value="missing">
                      ناقصة ({missing.length})
                    </TabsTrigger>
                    <TabsTrigger value="invalid">
                      غير صالحة ({invalid.length})
                    </TabsTrigger>
                    <TabsTrigger value="all">
                      الكل ({auditResults.equipment.length})
                    </TabsTrigger>
                  </TabsList>
      ...
      <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={converting}>
              إلغاء
            </AlertDialogCancel>
            <AlertDialogAction
