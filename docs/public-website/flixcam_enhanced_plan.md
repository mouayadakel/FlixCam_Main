...
export class PricingService {
  ...
  async calculateBookingPrice(params: {
    ...
    if (params.deliveryMethod === 'DELIVERY') {
      breakdown.deliveryFee = await this.calculateDeliveryFee(
  ...
  private async calculateItemCost(
    ...
    if (days >= 7 && equipment.weeklyRate) {
      const weeklyTotal = 
        Math.floor(days / 7) * equipment.weeklyRate +
        (days % 7) * equipment.dailyRate;
    ...
    if (days >= 30 && equipment.monthlyRate) {
      const monthlyTotal = 
        Math.floor(days / 30) * equipment.monthlyRate +
        (days % 30) * equipment.dailyRate;
...
export class InventoryService {
  ...
  async checkAvailability(
    ...
    const bookedQuantity = equipment.bookingItems.reduce(
  ...
  async reserveInventory(
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of items) {
        ...
        if (!available) {
          throw new Error(
            `Equipment ${equipment.nameEn} is no longer available for selected dates`
          );
        }
      }
    });
...
export class BookingService {
  ...
  async processLateReturn(bookingId: string): Promise<void> {
    ...
    if (today > dueDate && booking.status === 'ACTIVE') {
      ...
      const dailyRate = booking.items.reduce(
