...
const PERMISSIONS: Array<{ name: string; description?: string; categoryName: string; sortOrder: number }> = [
  ...
  { name: 'booking.*', categoryName: 'booking', sortOrder: 0, description: 'All booking permissions' },
  { name: 'equipment.*', categoryName: 'equipment', sortOrder: 0, description: 'All equipment permissions' },
  { name: 'client.*', categoryName: 'client', sortOrder: 0, description: 'All client permissions' },
  { name: 'invoice.*', categoryName: 'invoice', sortOrder: 0, description: 'All invoice permissions' },
  { name: 'payment.*', categoryName: 'payment', sortOrder: 0, description: 'All payment permissions' },
  { name: 'contract.*', categoryName: 'contract', sortOrder: 0, description: 'All contract permissions' },
  { name: 'quote.*', categoryName: 'quote', sortOrder: 0, description: 'All quote permissions' },
  { name: 'reports.*', categoryName: 'reports', sortOrder: 0, description: 'All report permissions' },
  { name: 'warehouse.*', categoryName: 'warehouse', sortOrder: 0, description: 'All warehouse permissions' },
  { name: 'maintenance.*', categoryName: 'maintenance', sortOrder: 0, description: 'All maintenance permissions' },
  { name: 'delivery.*', categoryName: 'delivery', sortOrder: 0, description: 'All delivery permissions' },
  { name: 'category.*', categoryName: 'category', sortOrder: 0, description: 'All category permissions' },
...
const SYSTEM_ROLES = [
  { name: 'super_admin', displayName: 'Super Admin', displayNameAr: 'مدير النظام', description: 'Full system access', isSystem: true, color: '#dc2626', permissions: ['*'] },
  {
    name: 'admin',
    displayName: 'Admin',
    displayNameAr: 'مدير',
  ...
  },
  {
    ...
    isSystem: true,
    color: '#059669',
    permissions: ['invoice.*', 'payment.*', 'reports.read_financial', 'booking.read', 'client.read', 'dashboard.read'],
  },
  {
    name: 'data_entry',
    ...
    color: '#7c3aed',
    permissions: [
      'equipment.create', 'equipment.read', 'equipment.update_metadata', 'category.*', 'brand.*',
      'seo.edit_meta_titles', 'seo.edit_meta_descriptions', 'seo.edit_slugs', 'seo.edit_alt_text', 'studio.read',
    ],
  },
  {
    ...
    color: '#d97706',
    permissions: [
      'equipment.*', 'warehouse.*', 'maintenance.*', 'delivery.read', 'delivery.assign', 'booking.read', 'booking.update', 'reports.read_warehouse',
    ],
  },
  {
    name: 'delivery',
    ...
    color: '#0891b2',
    permissions: [
      'delivery.read', 'delivery.update_status', 'delivery.complete', 'booking.read', 'equipment.read', 'client.read', 'warehouse.check_out',
    ],
  },
  {
    name: 'technician',
...
const CUSTOM_ROLES: Array<{ name: string; displayName: string; displayNameAr: string; description?: string; permissions: string[] }> = [
  { name: 'sales_manager', displayName: 'Sales Manager', displayNameAr: 'مدير المبيعات', permissions: ['quote.*', 'booking.*', 'client.read', 'client.update', 'invoice.read', 'dashboard.read'] },
  { name: 'customer_service', displayName: 'Customer Service', displayNameAr: 'خدمة العملاء', permissions: ['booking.read', 'booking.update', 'client.read', 'client.update', 'quote.read', 'invoice.read'] },
  { name: 'marketing_manager', displayName: 'Marketing Manager', displayNameAr: 'مدير التسويق', permissions: ['marketing.*', 'coupon.*', 'client.read', 'reports.read', 'dashboard.read'] },
  { name: 'risk_manager', displayName: 'Risk Manager', displayNameAr: 'مدير المخاطر', permissions: ['approval.*', 'client.read', 'booking.read', 'payment.read', 'ai.risk_assessment'] },
  { name: 'approval_agent', displayName: 'Approval Agent', displayNameAr: 'وكيل الموافقات', permissions: ['approval.read', 'approval.approve', 'approval.reject', 'booking.read', 'client.read'] },
  { name: 'auditor', displayName: 'Auditor', displayNameAr: 'مراجع حسابات', permissions: ['audit.read', 'audit.export', 'reports.read', 'booking.read', 'invoice.read', 'payment.read'] },
  { name: 'ai_operator', displayName: 'AI Operator', displayNameAr: 'مشغل الذكاء الاصطناعي', permissions: ['ai.*', 'equipment.read', 'category.read', 'dashboard.read'] },
]

const ROLE_CONFLICTS: Array<{ roleA: string; roleB: string; reason: string }> = [
  { roleA: 'finance', roleB: 'data_entry', reason: 'Separation of duties: Finance and Data Entry cannot be combined' },
  { roleA: 'warehouse_manager', roleB: 'finance', reason: 'Separation of duties: Warehouse and Finance' },
  { roleA: 'data_entry', roleB: 'admin', reason: 'Data Entry is a restricted role; Admin includes broader access' },
]
...
const MENU_ITEMS: Array<{
  ...
  { name: 'command_center', label: 'Command Center', labelAr: 'مركز القيادة', sortOrder: 1, permissions: [] },
  { name: 'dashboard', label: 'Dashboard', labelAr: 'لوحة التحكم', href: '/admin/dashboard', parentName: 'command_center', sortOrder: 1, permissions: ['dashboard.read'] },
  { name: 'action_center', label: 'Action Center', labelAr: 'مركز الإجراءات', href: '/admin/action-center', parentName: 'command_center', sortOrder: 2, permissions: ['dashboard.read'] },
  { name: 'approvals', label: 'Approvals', labelAr: 'الموافقات', href: '/admin/approvals', parentName: 'command_center', sortOrder: 3, permissions: ['approval.read'] },
  { name: 'live_ops', label: 'Live Operations', labelAr: 'العمليات الحية', href: '/admin/live-ops', parentName: 'command_center', sortOrder: 4, permissions: ['dashboard.read'] },
  { name: 'booking_engine', label: 'Booking Engine', labelAr: 'محرك الحجوزات', sortOrder: 2, permissions: [] },
  { name: 'quotes', label: 'Quotes', labelAr: 'عروض الأسعار', href: '/admin/quotes', parentName: 'booking_engine', sortOrder: 1, permissions: ['quote.read'] },
  { name: 'bookings', label: 'Bookings', labelAr: 'الحجوزات', href: '/admin/bookings', parentName: 'booking_engine', sortOrder: 2, permissions: ['booking.read'] },
  { name: 'recurring_bookings', label: 'Recurring Bookings', labelAr: 'الحجوزات المتكررة', href: '/admin/recurring-bookings', parentName: 'booking_engine', sortOrder: 3, permissions: ['booking.read'] },
  { name: 'calendar', label: 'Calendar', labelAr: 'التقويم', href: '/admin/calendar', parentName: 'booking_engine', sortOrder: 4, permissions: ['booking.read'] },
  { name: 'smart_sales', label: 'Smart Sales Tools', labelAr: 'أدوات البيع الذكية', sortOrder: 3, permissions: [] },
  { name: 'ai_features', label: 'AI Features', labelAr: 'ميزات الذكاء الاصطناعي', href: '/admin/ai', parentName: 'smart_sales', sortOrder: 1, permissions: ['ai.use'] },
  { name: 'kit_builder', label: 'Kit Builder', labelAr: 'منشئ الحزم', href: '/admin/kit-builder', parentName: 'smart_sales', sortOrder: 2, permissions: ['kit.read', 'ai.kit_builder'] },
  { name: 'dynamic_pricing', label: 'Dynamic Pricing', labelAr: 'التسعير الديناميكي', href: '/admin/dynamic-pricing', parentName: 'smart_sales', sortOrder: 3, permissions: ['pricing.read', 'ai.pricing'] },
  { name: 'inventory_assets', label: 'Inventory & Assets', labelAr: 'المخزون والأصول', sortOrder: 4, permissions: [] },
  { name: 'equipment', label: 'Equipment', labelAr: 'المعدات', href: '/admin/inventory/equipment', parentName: 'inventory_assets', sortOrder: 1, permissions: ['equipment.read'] },
  { name: 'kits', label: 'Kits & Bundles', labelAr: 'الحزم (المجموعات)', href: '/admin/inventory/kits', parentName: 'inventory_assets', sortOrder: 2, permissions: ['kit.read'] },
  { name: 'categories', label: 'Categories', labelAr: 'الفئات', href: '/admin/inventory/categories', parentName: 'inventory_assets', sortOrder: 3, permissions: ['category.read'] },
  { name: 'brands', label: 'Brands', labelAr: 'العلامات التجارية', href: '/admin/inventory/brands', parentName: 'inventory_assets', sortOrder: 4, permissions: ['brand.read'] },
  { name: 'studios', label: 'Studios', labelAr: 'الاستوديوهات', href: '/admin/studios', parentName: 'inventory_assets', sortOrder: 5, permissions: ['studio.read'] },
  { name: 'import', label: 'Import', labelAr: 'الاستيراد', href: '/admin/inventory/import', parentName: 'inventory_assets', sortOrder: 6, permissions: ['import.read'] },
  { name: 'field_ops', label: 'Field Operations', labelAr: 'العمليات الميدانية', sortOrder: 5, permissions: [] },
  { name: 'warehouse', label: 'Warehouse', labelAr: 'المستودع', href: '/admin/ops/warehouse', parentName: 'field_ops', sortOrder: 1, permissions: ['warehouse.read'] },
  { name: 'delivery', label: 'Delivery', labelAr: 'التوصيل', href: '/admin/ops/delivery', parentName: 'field_ops', sortOrder: 2, permissions: ['delivery.read'] },
  { name: 'technicians', label: 'Technicians', labelAr: 'الفنيون', href: '/admin/technicians', parentName: 'field_ops', sortOrder: 3, permissions: ['user.read'] },
  { name: 'maintenance', label: 'Maintenance', labelAr: 'الصيانة', href: '/admin/maintenance', parentName: 'field_ops', sortOrder: 4, permissions: ['maintenance.read'] },
  { name: 'damage_claims', label: 'Damage Claims', labelAr: 'مطالبات الأضرار', href: '/admin/damage-claims', parentName: 'field_ops', sortOrder: 5, permissions: ['booking.read'] },
  { name: 'finance_legal', label: 'Finance & Legal', labelAr: 'المالية والقانونية', sortOrder: 6, permissions: [] },
  { name: 'invoices', label: 'Invoices', labelAr: 'الفواتير', href: '/admin/invoices', parentName: 'finance_legal', sortOrder: 1, permissions: ['invoice.read'] },
  { name: 'payments', label: 'Payments', labelAr: 'المدفوعات', href: '/admin/payments', parentName: 'finance_legal', sortOrder: 2, permissions: ['payment.read'] },
  { name: 'contracts', label: 'Contracts', labelAr: 'العقود', href: '/admin/contracts', parentName: 'finance_legal', sortOrder: 3, permissions: ['contract.read'] },
  { name: 'financial_reports', label: 'Financial Reports', labelAr: 'التقارير المالية', href: '/admin/finance/reports', parentName: 'finance_legal', sortOrder: 4, permissions: ['reports.read_financial'] },
  { name: 'analytics', label: 'Analytics & Utilization', labelAr: 'التحليلات والإشغال', href: '/admin/analytics', parentName: 'finance_legal', sortOrder: 5, permissions: ['dashboard.analytics'] },
  { name: 'crm_marketing', label: 'CRM & Marketing', labelAr: 'العملاء والتسويق', sortOrder: 7, permissions: [] },
  { name: 'clients', label: 'Clients', labelAr: 'العملاء', href: '/admin/clients', parentName: 'crm_marketing', sortOrder: 1, permissions: ['client.read'] },
  { name: 'reviews', label: 'Reviews', labelAr: 'التقييمات', href: '/admin/reviews', parentName: 'crm_marketing', sortOrder: 2, permissions: ['client.read'] },
  { name: 'customer_segments', label: 'Customer Segments', labelAr: 'شرائح العملاء', href: '/admin/settings/customer-segments', parentName: 'crm_marketing', sortOrder: 3, permissions: ['client.read'] },
  { name: 'coupons', label: 'Coupons', labelAr: 'الكوبونات', href: '/admin/coupons', parentName: 'crm_marketing', sortOrder: 4, permissions: ['coupon.read'] },
  { name: 'marketing', label: 'Marketing', labelAr: 'التسويق', href: '/admin/marketing', parentName: 'crm_marketing', sortOrder: 5, permissions: ['marketing.read'] },
  { name: 'settings_section', label: 'Settings', labelAr: 'الإعدادات', sortOrder: 8, permissions: [] },
  { name: 'general_settings', label: 'General Settings', labelAr: 'الإعدادات العامة', href: '/admin/settings', parentName: 'settings_section', sortOrder: 1, permissions: ['settings.read'] },
  { name: 'notification_templates', label: 'Notification Templates', labelAr: 'قوالب الإشعارات', href: '/admin/settings/notification-templates', parentName: 'settings_section', sortOrder: 2, permissions: ['settings.read'] },
  { name: 'integrations', label: 'Integrations', labelAr: 'التكاملات', href: '/admin/settings/integrations', parentName: 'settings_section', sortOrder: 3, permissions: ['settings.read'] },
  { name: 'features', label: 'Features', labelAr: 'الميزات', href: '/admin/settings/features', parentName: 'settings_section', sortOrder: 4, permissions: ['settings.update'] },
  { name: 'roles', label: 'Roles', labelAr: 'الأدوار', href: '/admin/settings/roles', parentName: 'settings_section', sortOrder: 5, permissions: ['settings.manage_roles'] },
  { name: 'ai_control', label: 'AI Control', labelAr: 'التحكم بالذكاء الاصطناعي', href: '/admin/settings/ai-control', parentName: 'settings_section', sortOrder: 6, permissions: ['settings.update'] },
]
...
async function main() {
  ...
  for (const m of MENU_ITEMS) {
    ...
    const created = await prisma.menuItem.upsert({
      where: { name: m.name },
      update: { label: m.label, labelAr: m.labelAr, href: m.href ?? null, parentId, sortOrder: m.sortOrder, icon: m.icon ?? null },
      create: {
        name: m.name,
        label: m.label,
        labelAr: m.labelAr,
